...
Plugin URI: http://orbisius.com/products/wordpress-plugins/orbisius-quick-nav/?utm_source=orbisius-quick-nav&utm_medium=plugin-header&utm_campaign=product
...
function orbisius_quick_nav_enqueue_chosen_jquery() {
    wp_register_style( 'orbisius_quick_nav_chosen_css', '//cdnjs.cloudflare.com/ajax/libs/chosen/1.4.2/chosen.min.css', true );
    wp_enqueue_style( 'orbisius_quick_nav_chosen_css' );

    wp_enqueue_script( 'jquery' );
    wp_register_script( 'orbisius_quick_nav_chosen_js', '//cdnjs.cloudflare.com/ajax/libs/chosen/1.4.2/chosen.jquery.min.js', array( 'jquery' ) );
    wp_enqueue_script( 'orbisius_quick_nav_chosen_js' );
}
...
function orbisius_quick_nav_admin_inline_js() {
    ...
    var orbisius_quick_nav_cfg = $json;

    jQuery(function($) {
        
        try { // cdn is offline or the computer is not connected.
            $('[name=orb_quick_nav_select]').chosen({disable_search_threshold: 10});
        } catch (e) {}
 ...
 * @param array $ctx
 */
function orbisius_quick_nav_render_dropdown_item_label( $label, $ctx ) {
     $label .= " (id {$ctx['item']['id']})";

    if ($ctx['item']['post_status'] != 'publish') {
        $label .= ' [' . $ctx['item']['post_status'] . ']';
    }

    return $label;
}

add_filter( 'orbisius_quick_nav_dropdown_item_label', 'orbisius_quick_nav_render_dropdown_item_label', 10, 2 );

/**
 *
 * @param obj $post
 */
function orbisius_quick_nav_gen_dropdown($post = null) {
    if ( ! empty( $post->post_type ) ) {
        $post_type = $post->post_type;
    } elseif ( ! empty( $_REQUEST['post_type'] ) ) {
        $post_type = wp_kses( $_REQUEST['post_type'], array() );
    ...
    $buff = "<div id='orbisius_quick_nav_dropdown_container' style='padding:3px;'>\n";
    $buff .= "Orbisius Quick Nav To: \n";

    $statuses = get_post_stati();
    
    $args = array(
        'name' => 'orb_quick_nav_select',
        'id' => 'orb_quick_nav_select_' . $post_type . '_id',
        'depth'            => 0,
        'child_of'         => 0,
        'echo'             => 0,
        'post_type'        => $post_type,
    ...
    if ($post_type == 'page') {
        ...
        $items_raw = orbisius_quick_nav_util::get_items($post_type, array( 'post_status' => $statuses) );
        ...
        foreach ($items_raw as $rec) {
            ...
            $dropdown_options[$rec['id']] = apply_filters( 'orbisius_quick_nav_dropdown_item_label', $rec['post_title'], array( 'item' => $rec ) );
        }
        ...
        $where = sprintf( "WHERE post_status != 'trash' AND post_type = '%s' ", esc_sql($post_type) );
        $order = 'ORDER BY post_title DESC';
        
        $items_raw = $wpdb->get_results("SELECT id, post_title, post_status FROM {$wpdb->posts} $where $order LIMIT $limit", ARRAY_A);
...
//    $buff .= "| <a href='https://wordpress.org/support/view/plugin-reviews/orbisius-quick-nav' title='Go to the plugin&#39;s review page. Opens in a new tab/window' target='_blank'>Write a Review</a>\n";
//    $buff .= "</div>\n";
...
function orbisius_quick_nav_setup_admin() {
	...
	add_options_page( 'Orbisius Quick Nav', 'Orbisius Quick Nav', 'manage_options', __FILE__, 'orbisius_quick_nav_options_page' );
...
function orbisius_quick_nav_register_settings() { // whitelist options
    ...
    if ( ! empty( $pagenow ) && in_array( $pagenow, $supported_pages ) ) {
        global $wp_version;

        // depending on theversion we'll use the right hook
        ...
        if ( empty( $_REQUEST['post'] ) ) {
            add_action( 'all_admin_notices', 'orbisius_quick_nav_gen_dropdown' );
        } else {
            add_action( $hook, 'orbisius_quick_nav_gen_dropdown' );
        }
        
        add_action( 'admin_footer', 'orbisius_quick_nav_admin_inline_js' );
        add_action( 'admin_enqueue_scripts', 'orbisius_quick_nav_enqueue_chosen_jquery' );
    }
}
            ...
            <div id="post-body" class="metabox-holder columns-2">
                    ...
                    <div class="meta-box-sortables ui-sortable">
                        ...
                        <div class="postbox">
                            <div class="inside">
                                ...
                                It will show a dropdown that you can use to switch to the desired page/post/woocommerce product etc.
                        ...
                        <div class="postbox">
                            ...
                            <div class="inside">
                                We'd really appreciate it if you write review about our plugin. If you think we can improve it in any way let us know at
                                <strong>help@orbisius.com</strong>
                                <br/>
                                <a href='https://wordpress.org/support/view/plugin-reviews/orbisius-quick-nav' 
                                   title='Go to the plugin&#39;s review page. Opens in a new tab/window' target='_blank'
                                   class="button-primary">Write a Review</a>
                            </div> <!-- .inside -->
                        </div> <!-- /Demo -->

                        <!-- Demo -->
                        <div class="postbox">
                            <h3><span>Demo</span> | <a href='http://www.youtube.com/watch?v=RK3SIoAsM5g' target="_blank" title="Opens in a new tab/window">Watch it on YouTube</a></h3>
                            <div class="inside">
                                <iframe width="560" height="315" src="http://www.youtube.com/embed/RK3SIoAsM5g" frameborder="0" allowfullscreen></iframe>
                            </div> <!-- .inside -->
                        </div> <!-- /Demo -->
                ...
                <div id="postbox-container-1" class="postbox-container">
                    ...
                    <div class="meta-box-sortables">
                        ...
                        <!-- Staging -->
                        <div class="postbox">
                            <h3><span>Free Test/Staging Site</span></h3>
                            <div class="inside">
                                <div class="">
                                    Did a plugin or theme crash your website? Try <a href="//qsandbox.com/?utm_source=orbisius-quick-nav&utm_medium=settings_screen&utm_campaign=product"
                                        target="_blank" title="[new window]">http://qSandbox.com</a> - it
                                        allows you to set up a free test/sandbox WordPress site in seconds or import (<span title='Paid feature'>$</span>) your existing one.
                                </div>
                            </div> <!-- .inside -->
                        </div> <!-- .postbox -->
                        <!-- /Staging -->
                        ...
                        <div class="postbox">
                            ...
                            <div class="inside">
                                Do you need this or other plugin customized?
                                <br/><a href="//orbisius.com/page/free-quote/?utm_source=<?php echo str_replace('.php', '', basename($plugin_file));?>&utm_medium=plugin-settings&utm_campaign=product"
                        ...
                        <div class="postbox">
                            <div class="inside">
                                ...
                                <span>
                                    ...
                                    ?>">Product Page</a>
                                    |
                                    <a href="//orbisius.com/forums/forum/community-support-forum/wordpress-plugins/<?php echo $pl_slug;?>/?utm_source=<?php
                                        echo $pl_slug;?>&utm_medium=plugin-settings&utm_campaign=product"
                                    target="_blank" title="[new window]">Forums</a>
                                    |
                                    More <a href="//orbisius.com/products/?utm_source=<?php
